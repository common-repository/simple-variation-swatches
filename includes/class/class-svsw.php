...
if ( ! class_exists( 'SVSW' ) ) {
	...
	class SVSW {
		...
		public function atts_to_swatch( $attribute_name, $options, $show_name ) {
			...
			$anc = isset( $this->data['att_name_color'] ) ? $this->data['att_name_color'] : '';
			$anb = isset( $this->data['att_name_background'] ) ? $this->data['att_name_background'] : '';
			...
			<div class="svsw-wrap <?php echo esc_attr( $block_design ); ?>">
				<?php if ( $show_name ) : ?>
					<?php 
						...
						do_action( 'svsw_after_att_name', $attribute_name, $this->pack );
					?>
					</label>
				<?php endif; ?>
		...
		public function skipped_atts( $skipped_terms, $attribute_name ) {
			...
			if ( isset( $this->data['svsw_font_size'] ) && ! empty( $this->data['svsw_font_size'] ) ) {
				$font_size = 'font-size: ' . esc_attr( $this->data['svsw_font_size'] ) . 'px;';
			}
		...
		public function render_swatch( $slug, $type, $value ) {
			...
			if ( 'color' === $type ) {
				...
				<div class="svsw-swatch svsw-swatch-radio" data-term="<?php echo esc_attr( $slug ); ?>">
					<input type="radio" name="svsw_radio_swatch_<?php echo esc_attr( $this->taxonomy ); ?>" value="<?php echo esc_html( $slug ); ?>">
